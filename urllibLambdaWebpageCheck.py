# Original https://github.com/TacMechMonkey/urllib-lambda-webpage-check
...
# Global variables to be set by SysAdmin
url = "https://outlook123.com/CONTOSSO.com"
...
# Input AWS SNS ARN
snsarn = 'arn:aws:sns:ap-southeast-2:XXXXXXXXXXXXX:Daily_Check_Notifications'
    ...
    try:
        ...
        with urllib.request.urlopen(request) as response:
            ...
            if htmllength // htmlchar != 1:
                error_list.append(str("Page HTML Error"))
            
            if adfslink in str(acturl):
                result = "passed."
            else:
                result = "FAILED!"
            ...
            responsedata = ""
            notification(result, msgdet_list, error_list)

    # Throwaway exception handler, use with caution
    except ValueError:
        result = failed
     
# Use AWS SNS to create a notification email with the additional data generated
def notification(result, msgdet_list, error_list):
    datacheck = str("\n".join(msgdet_list))
    errorcheck = str("\n".join(error_list))
    notificationbody = str(errorcheck + datacheck)
    # Confirm notificationheader formatting in CloudWatch. /
    # If it's messy, move the it directly to "Subject = " below
    notificationheader = ("\nThe daily ADFS check has been marked as", result, "\n")
